...
import { qty, stockValue } from "../../lib/selectors";
...
export default function ItemsStock() {
  ...
  const zeroSomewhere = stocked.filter((k) => ALL_LOCS.some((l) => qty(s, l, k) <= 0)).length;
  ...
  const rows = keys
    ...
    .map((k) => {
      ...
      return { k, per, tot, value: tot * IT[k].cost };
    });
      ...
      <Kpis
        items={[
          ...
          { l: "Items tracked", v: String(keys.length), d: <>{stocked.length} stocked · {keys.length - stocked.length} made to order</> },
          {
            ...
            d: <>At least one of the five locations is dry</>,
          },
          {
        ...
        <DataTable
          ...
          rows={rows.map((r) => ({
            ...
            cells: [
              ...
              ...ALL_LOCS.map((l, i) => {
                const v = r.per[i];
                if (v <= 0) return <span className="dim">–</span>;
                if (l === "store" && IT[r.k].rl > 0 && v <= IT[r.k].rl)
                  return <span style={{ color: "var(--warn)" }} title={`Reorder level ${IT[r.k].rl}`}>{fq(v, r.k)}</span>;
                return <>{fq(v, r.k)}</>;
              }),
              r.tot > 0 ? <b>{fq(r.tot, r.k)}</b> : <span className="dim">–</span>,
              r.value > 0 ? money0(r.value) : <span className="dim">–</span>,
            ],
          }))}
          empty={{
