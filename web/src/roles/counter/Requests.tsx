...
import { sum, U } from "../../lib/fmt";
import {
  Alert, Btn, BtnRow, Card, DataTable, Field, FormRow, PageHead, Section,
...
export default function Requests() {
  ...
  const usable = draft.filter((l) => l.it && l.qty > 0).length;
    ...
    <>
      ...
      <Card title="New request" sub={`From ${L.n} (${L.c}) · raised by ${user.n}`}
        ...
        <FormRow cols="f2">
          ...
          <Field label="Lines ready" hint="Only lines with an item and a quantity above zero are sent.">
            <input readOnly value={`${usable} of ${draft.length}`} />
          </Field>
        </FormRow>
      ...
      <Card flush>
        ...
        <TableFoot count={rows.length}
          extra={<>{L.n} · {L.c} · {openCount} awaiting the outlet manager</>} />
      </Card>
