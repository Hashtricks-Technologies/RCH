...
const balanceOf = (o: PurchaseOrder) =>
  o.lines
    ...
    .map((l) => ({ it: l.it, qty: Math.round((l.qty - l.recv) * 1000) / 1000 }));
...
export default function PurchaseOrders() {
  ...
  const orderedValue = sum(s.po.filter((o) => o.st === "Ordered"), poValue);
  ...
  const overSlab = s.po.filter((o) => o.needsApproval).length;
  ...
  return (
    <>
      ...
      <Kpis items={[
        ...
        { l: "Value on order", v: money0(orderedValue), d: `${ordered.length} order(s) placed, none received yet` },
