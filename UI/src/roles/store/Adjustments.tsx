...
/** The two shelves the store keeper answers for, and the only two the server lets it adjust: the
 *  central store, and the rejected-goods shelf - what a goods receipt turned away sits there until
 *  somebody destroys it or sends it back. The kitchen adjusts its own; an outlet's is corrected
 *  only through a counter's request the manager approves. */
const SHELVES: [StockLoc, ...StockLoc[]] = [STORE, QUARANTINE];
