import { isAccountTender, istDate } from "@rch/domain";
import type { Bill, Dated, Tender, Tone } from "../../types";
...
/** Which of the three a tender lands in. `account` is `isAccountTender` (@rch/domain) - the one
 *  table that says which tenders run up a balance, the same one the sale and the receivables
 *  query read, so this list cannot drift from theirs when a tender is added. */
export const settlementOf = (pay: string): Settlement =>
  pay === "Cash" ? "drawer" : isAccountTender(pay as Tender) ? "account" : "bank";
...
export const voidableToday = (b: Dated<Bill>, now: Date = new Date()): boolean =>
