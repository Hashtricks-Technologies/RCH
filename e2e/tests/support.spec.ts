...
test("a ticket is raised, answered, resolved and rated — and stays the raiser's own", async ({ page, browser }) => {
  ...
  // And it is nobody else's business. The desk is on the server now, so this is a real question
  // to ask of it: a second browser, a second account, and the ticket the first one raised is not
  // in the list the server sends back.
