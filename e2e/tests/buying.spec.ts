...
test("a requisition becomes an order, and the delivery splits between two shelves", async ({ browser }) => {
  ...
  // The server's own arithmetic and its own wording, three decimals and all: 60 arrived, 3 were
  // turned away, 57 reached the shelf.
  ...
  await expect(quarantine.locator("tbody tr", { hasText: MILK })).toContainText("3");
});
