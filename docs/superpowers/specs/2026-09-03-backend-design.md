...
| §10 UI | The store's ledger-report foot totals each column with `unitTotal`, not `ledgerTotals` (`packages/domain`); `ledgerTotals` was deleted with it, keeping only its own package's test as a caller. | The central store's ledger carries litres, kilos and countable things on one page — summing them across units is exactly the "adding litres to cups" defect the root guide forbids. |
...
| §9.2 `lib/tickets.ts` | `readTicket`'s `otp` field carries a status check (`Issued` → the row's own value, else `""`) whose `Issued` branch is unreachable today — `readTicket`'s only caller (`reread` in `modules/tickets/service.ts`) is always reached *after* a status change away from `Issued`. Written this way so the field stays right rather than accidentally right. | A future caller that rereads a live `Issued` ticket would owe it the same location-and-role check `redactOtps` makes, since `readTicket` is handed only an id, no `who`. Flagged here so that caller does not ship without the check. |

**Landing in the Phase 6 fix wave — the intended end state, not yet on this branch when Task 11 wrote this table:**

| Section | Amendment | Why |
|---|---|---|
| §6, §9.1 (fix wave) | The OTP is minted at random when a ticket is created (`crypto.randomInt(100000, 1000000)` in `allocateTicket`, `apps/api/src/lib/tickets.ts`), not derived from the ticket number. `makeOtp` — a pure, publicly exported function of the ticket's own sequence number — is deleted from `packages/domain`'s surface. This reverses Phase 3's own acceptance (§16, Phase 3: "The OTP is stored and served in the clear... `makeOtp` is an operational check, not a security token"). | A formula the browser can run against a number already on its own screen is not a redaction, however carefully the wire withholds the field. The whole-branch review that found this named it blocking: without this, the store's own screens carry the code even after §16's redaction rows landed, because the code was never secret, only unlisted. |
| §5.1 (fix wave) | `PAR_FACTOR` moves from `packages/contract` to `packages/domain`. `ALL_LOCS` and `OUTLETS` stay in `packages/contract` (the server reads them). `packages/contract/src/fixtures/master.ts`'s now-unread re-export lines for all three are deleted. | `PAR_FACTOR` has no server-side rule reading it — it is UI tuning, the same class of constant every other file in `packages/domain` already holds, not a wire shape. It passed through the contract package for one task's duration only because it moved out of the fixtures alongside two constants that do belong there. |
| §10 UI (fix wave) | `auth` (`store/index.ts`) gains a fifth state, `"failed"`. A sign-in or a reload whose `GET /snapshot` fails (not a 401 — `onSessionLost` already covers that) renders a full-page retry instead of falling back to `"ready"` with "Could not load the latest data — showing what is in memory." | There is no memory to fall back to any more — `data/seed.ts` is gone — so the old sentence described a state that stopped being true the moment the browser's own copy of the hospital was deleted. |
| §9.2/§10 production (fix wave) | The kitchen's outbound handover (`roles/prod/Tickets.tsx`) gains the same OTP field and labelled-override choice the store's issue desk has always had, rather than calling `handover(t.id)` with no OTP unconditionally. | Every kitchen handover ran silently as the supervisor override before this landed — not a deliberate choice, an oversight the screen never surfaced a control for. |
| §10 UI (fix wave) | The manager's transfers table (`roles/manager/ItemsStock.tsx`) gains `Cancelled` as a selectable stage filter, alongside Reserved / In transit / Received. | The label already rendered correctly for a cancelled shop-to-shop ticket; only the filter list had not caught up to the status the table could already show. |
| §6, §9.1 (fix wave) | `redactOtps` requires the caller's **role** to be `counter`, `prod` or `store`, in addition to standing at the ticket's `to` — see the accepted row above for the shape; this is the fix-wave completion of it, closing the gap where an outlet manager's own `loc` could coincide with a ticket's `to`. | Recorded twice deliberately: once as the rule this phase intended, once here as the fix-wave completion, because the two landed in different commits and a reader diffing the branch should find both. |
| §11.4 (fix wave) | `.github/workflows/deploy.yml`'s "Prove it came up" step gains an explicit `timeout-minutes` at the job level, and its verification uses `kubectl wait --for=condition=Available` in place of a bare `curl` with no `--max-time`, so a stuck rollout or an unreachable image registry cannot hang the job indefinitely. | Found in the whole-branch review: the step as first written had no bound on how long a hung pull or a wedged pod could hold the runner. |
| §13 e2e (fix wave) | `E2E_OTP_REDACTED` and `E2E_SUPPORT_SERVER` — the two switches that gated `e2e/tests/{request-chain,support}.spec.ts`'s assertions behind server-side support for the OTP redaction and the support desk, both landed mid-phase in earlier waves — are deleted from `ci.yml`'s kind-install step and from the specs themselves; the assertions they gated are unconditional. | Both features they were waiting for are on `develop` now; a switch nothing turns off any more is dead weight in the CI config and an invitation to wonder what it still does. |
