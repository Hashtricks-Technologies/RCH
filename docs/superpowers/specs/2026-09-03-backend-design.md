...
## 16. Amendments recorded during Phases 1–6 (2026-09-04)

Decisions taken while executing Phases 1–6 that refine or correct the sections above.
...
### Amendments recorded during Phase 6 (2026-09-04)
...
| §9.2 support, sentences | Three new refusals, not four: a status only support may set (`Only support moves a ticket to <st> — you can mark it resolved or close it`), a rating on an unfinished ticket (`<id> is not finished yet — rate it once support has resolved it`), and a reply to a closed one (`<id> is closed — raise a new ticket if it has come back`). `setTicketStatus`'s *success* sentence (`<id> — <status>`) is `ops.ts:110` verbatim, not new. | The browser refused none of the three because it had no rules; the wording is recorded so the screens repeat the server rather than inventing a second voice, and the success-line miscount is corrected here rather than repeated. |
...
| §7.2 `tickets` | `TicketSchema` gains `hist`, read from `document_history`, required rather than optional. `GET /documents/:type/:id/history` is still not built. | The rows have been written and unread since Phase 3. The generic endpoint would have to answer for eleven document types and be scoped for each; a field on the one document that needed it is smaller and complete. |
| §6, §9.1 | The OTP is on the wire only while a ticket is `Issued` and only for a caller at its `to`. | The store's issue desk printed it beside the box that verifies it, which is not a check. The supervisor override, refused to any other role and recorded in a history that is now visible, is the way past an absent collector. |
| §6, §9.1 | The role check is narrower than the location check alone: a caller must be `counter`, `prod` or `store` **and** stand at the ticket's `to`. | Without it, a manager whose own `loc` happened to equal a ticket's `to` (an outlet manager, who is not location-scoped, standing "at" any outlet) could read the code too — the location check alone was not sufficient, only necessary. |
...
| §9.2 `handover`, sentence | `"Handed over"` (no suffix) is a new trail sentence for an ordinary, non-override handover — Phase 3 only ever wrote a row for the override, so there was no earlier wording to reuse. | The trail now covers a ticket's whole life, not just its two exceptional moments, and a plain handover needed a word of its own. |
| §9.2 ownership shape, tickets vs support | A support write refuses somebody else's ticket with **404** (`There is no support ticket <id>.`); `cancelTicket` refuses the wrong outlet with **403** (`You can only do this for the location the ticket is issued from.`). Both stand, deliberately different. | A support ticket you did not raise is not yours to know exists — the same shape a missing sidebar module has. A ticket's location, by contrast, is visible to the role in the first place (it is *this* ticket, just the wrong end of it); refusing the row is a location check, the same one every other location-scoped write answers with 403. |
| §9.1 | `SnapshotSchema` gains `roster`, from `payers` where active, ordered by name. Not scoped by role or location. | The till validated against the table and the browser read a fixture; a patient admitted after the last build was unbillable. Every counter bills every kind of payer, so there is nothing to narrow. |
...
| §9.1 `stockLedger` | No per-location narrowing: `store`, `manager`, `buyer` and `prod` may each read any location's ledger, including one that is not theirs. | The snapshot already gives every role every location's stock; a ledger over the same data is not a new leak, and `quarantine` has no owner to scope it to. |
| §9.1 `creditReport` | Any `counter` may read any payer's credit, hospital-wide, not only a payer they have billed. | There is only one figure per payer — the ceiling is a fact about the person, not about a till — so there is no narrower true answer to give. |
| §7.2 `bills_staff_credit_idx` | The index stays partial on `payer_kind = 'staff'`; a `patient`/`dept` credit read is therefore a sequential scan of `bills`, answering `0` every time (only a `Staff credit` tender ever creates credit). Accepted. | One home for "which payer kinds carry a ceiling" is worth a scan of a table this size; widening the index to cover an answer that is always zero buys nothing. |
...
| §10 UI | The store's ledger-report foot totals each column with `unitTotal`, not `ledgerTotals` (`packages/domain`), which was written in Task 6 and deleted in Task 10's fix round rather than left with no caller. | The central store's ledger carries litres, kilos and countable things on one page — summing them across units is exactly the "adding litres to cups" defect the root guide forbids. |
| §10 UI, known gap | `labelOf` (the "you are back on {home}" toast) resolves the signed-in role's own `NAV` first, falling back to any role's only for a shared key with no role-specific label. A counter denied at `/#/orders` still reads "Orders" — the kitchen's label — because the counter's own sidebar has no `orders` entry to prefer. Accepted, harmless. | The honest fix is a distinct route key per role for a shared path, which is a bigger change than a docs pass should make; recorded so it is not mistaken for an oversight. |
| §10 UI, known gap | A shop-to-shop transfer still has no counter-side handover screen: `routes.handover` admits `counter`, but no screen calls it from the receiving end — the ticket sits at `Issued`, reservation held, until the sending counter withdraws it or the store/kitchen hands it over on its behalf. | Phase 4 recorded the same gap and named Phase 6; Phase 6 closed the *cancel* half (the sending counter's own door) but not a *collect* half, because no screen was ever built for a counter collecting from another counter directly. Carried forward, not closed. |
| §9.2 support | Support message ids on the wire are `m1`, `m2`, … (the server stores `SUP-xxxx/m<n>`, `toWire` strips the prefix). Ordering ties on `(at, id)`, which sorts lexicographically above nine messages in one thread (`m10` before `m2`). Latent, accepted. | The desk has never carried a thread that long; the trigger for fixing it is the first one that does. |
| §9.2 `replyToTicket` | `mayReply` (`packages/domain/src/support.ts`) is not an edge in `SUPPORT_TRANSITIONS` — a reply is refused *before* it is written, so the transition table is never asked about a closed ticket. Both the service and the UI's reply box read the same predicate. | One rule, two consumers, the same shape every other domain predicate takes; the inline check that shipped in Task 5 moved here once the UI needed to preview it too. |
| §11.1 | Alerts ship as a `PrometheusRule` (`deploy/chart/rch/templates/prometheusrule.yaml`) gated on `serviceMonitor.enabled`. A Grafana dashboard JSON does **not** ship. | A dashboard in a ConfigMap is an unversioned blob nothing renders in CI and nothing fails when it drifts. |
| §11.1 | The `PrometheusRule` carries five alerts — the three metrics-derived ones spec §12 named (5xx rate, p95 latency, readiness) plus two new to this phase: `RchApiPoolSaturated` (the app's own Postgres pool, `pg_pool_waiting`/`pg_pool_idle`) and `RchSseListenerDown` (`sse_listener_up`, §10). The two RDS-CloudWatch rules (connections, free storage) stay runbook-only. | The pool and SSE-listener gauges have existed since Phase 3 with no alert reading them; wiring them into the chart the moment the mechanism to ship an alert existed was the obvious pairing. |
...
| §13 testing | `packages/domain/vitest.config.ts` and `packages/contract/vitest.config.ts` now pin `TZ=UTC`, matching `apps/api` and `UI`. `packages/domain`'s already carried the pin, added ahead of schedule in Phase 5 — only `packages/contract`'s actually needed the change. | Phase 5's Task 2 left the two unpinned because "the tests are TZ-independent"; the pin costs one line and removes the class of failure rather than the instance of it. |
| §13 testing | `apps/api/src/db/seed.ts` exports `seedDocuments(tx)` and `apps/api/src/test/db.ts` adds `resetDocuments(db)`: a per-case reset of 26 document and vendor tables that leaves master data, users and payers seeded once per file. | Suites re-seeding the whole hospital in `beforeEach` — master, items, recipes, menus, price lists, users, payers and every document — to get a clean document band dominate the run's wall clock for the invariant half they re-seed every time. `purchaseorders.test.ts` is the converted example; the rest are named candidates, not yet converted. |
| §14 | Phase 6 adds **no migration**. The journal ends at `0006_rate_contracts_live_uq`. | Every table it needed existed from Phase 1. |
| §14 row 6 | Phase 6 does **not** perform the first production deploy. It prepares `values-prod.yaml`, the workflow, the go-live checklist (`deploy/RUNBOOK.md` §11) and the exact commands, and stops. | Promotion needs a protected-branch push, `DEPLOY_ENABLED=true` and five AWS secrets, none of which the executor has or should have. It is the account owner's decision, not a phase's deliverable. |
| §14 row 6 exit | *"All §12 items verified on prod"* becomes: verified on a local stack and against the kind cluster, with six items marked **when promoted** — the readiness probe against real infrastructure, the alerts firing against real load, both RDS-CloudWatch rules, the restore drill's real-RDS half, the go-live checklist's own execution, and the production deploy itself. | Six of them cannot be observed before a production cluster exists. Marking them green would be the one way the exit check can lie. |
| §14 row 6 exit, §13 | *"smoke passes against prod"* becomes: the smoke runs against `pnpm dev` locally and against the kind cluster in CI, and is **never** pointed at production. It writes real bills, real tickets and real support tickets. | The chart CI installs is the chart production runs; a smoke against the hospital's ledger would sell stock nobody ordered. |
...
| §7.3 GRN | `GRN-<last 3 of PO>-<nn>` becomes `GRN-<yy><po number>-<nn>` (`PO-2026-0143` → `GRN-260143-01`), built by `grnId(poId, n)` in `packages/domain/src/ids.ts` rather than inline in the service. Old GRNs are not renumbered. | The three-character tail collides: `PO-2026-0143` and `PO-2027-0143` share it, and so do `PO-2026-0143` and `PO-2026-1143`. `grns.id` is a primary key, so a collision is a failed insert in the middle of a goods receipt — a 500 at the receiving door, not merely a wrong number. |
| §9.1, §9.2 tickets | No backfill for a ticket that existed before this phase: its trail has no `Issued` row, and reads from whichever Phase 6 write next touched it. | `document_history` for a ticket did not exist as a written-to trail before this phase; there is nothing to backfill it from. |
| §9.2 `lib/tickets.ts` | `readTicket` returns `otp: ""` unconditionally — it is handed an id and no `who`, so there is nothing to check a location or role against; a caller that ever needs the value reads the row itself under `redactOtps`' check. | Writing it this way rather than a status branch that happened to agree keeps the field right by construction rather than right by coincidence of who calls it today. |
| §11.1 `render.test.sh` | Every `!`-prefixed negated assertion in the chart's own test script was a no-op: bash's `set -e` never trips on a command whose exit status is inverted with a leading `!`, so eleven pre-existing checks plus the two Phase 6 added were silently passing whether or not the pattern they searched for was present. Replaced with a `refute` helper that does trip `set -e`. | The Phase 4 `warmPool` lesson again — a check that cannot fail is worse than none, because it reads as coverage that was never actually exercised. |
| §11.1 `prometheusrule.yaml` | The 5xx-rate alert's `description` field uses Sprig's `divf` (float division), not `div` (Sprig's `div` casts both operands to `int64`, so `div 1.0 0.01` renders as a division-by-zero template error rather than "100"). | Found while writing the alert's human-readable description; `divf` is the function that actually computes what the sentence claims. |
| §10 UI, known gap | The store's stock-ledger report drops any row whose item key is not in the browser's own catalogue (`known = rows.filter((r) => IT[r.it])`, `roles/store/Reports.tsx`) — a product that reached the ledger through a path the browser's registry has not caught up with (a live update mid-session, say) is silently absent from the printed report rather than shown with a placeholder name. | The report is a read, and the browser genuinely does not know the item's name or unit to render a row for it; recorded as a known gap rather than fixed, since a placeholder row ("unknown item") is a product decision, not a docs one. |
| §10 UI, known gap | A support ticket's timestamps (`t.at`, a message's `m.at`) render as bare `"HH:MM"` (`applySupportTickets`, `api/wire.ts`) with no date, the same as every other document's wire time — so a ticket raised a week ago still reads as though it happened at some point today. | Every other document in this system is acted on same-day or next-day, where `"HH:MM"` alone is unambiguous; a support ticket is the one document that can sit open for a genuinely long time, and nothing in `fmt.ts` was built to say "3 days ago" or a full date for the case where that matters. |

### Landed in the Phase 6 fix wave (2026-09-04)

Two commits, `a8f762b` ("Draw the collection OTP at random, and clear the review's remaining
list") and `19d486a` ("Give the kitchen an OTP box, and say so when there is nothing to show"),
merged into this branch as `7765b7f`. Recorded here rather than folded into the table above
because they landed after Task 11's own docs commit and needed a second pass to verify.

| Section | Amendment | Why |
|---|---|---|
| §6, §9.1 | The OTP is minted at random when a ticket is created (`crypto.randomInt(100000, 1000000)` in `allocateTicket`, `apps/api/src/lib/tickets.ts`), not derived from the ticket number. `makeOtp` and `packages/domain/src/otp.ts` are deleted outright — there is no longer a pure function of a ticket's sequence number anywhere in the tree. This reverses Phase 3's own acceptance (§16, Phase 3: "The OTP is stored and served in the clear... `makeOtp` is an operational check, not a security token"). | A formula the browser can run against a number already on its own screen is not a redaction, however carefully the wire withholds the field. The whole-branch review that found this named it blocking: without this, the store's own screens carried the code even after this phase's own redaction rows landed, because the code was never secret, only unlisted. |
| §6, §9.1 | `redactOtps` (`apps/api/src/modules/snapshot/scope.ts`) requires the caller's **role** to be `counter`, `prod` or `store` (the module-level `COLLECTS` set), in addition to standing at the ticket's `to`. | Without it, an outlet manager — not location-scoped, so their own `loc` is simply an outlet — reading an Issued ticket bound for their own home outlet would see the code too, which is exactly the leak the location check alone was meant to close. |
| §5.1 | `PAR_FACTOR` moves from `packages/contract` to `packages/domain/src/par.ts`. `ALL_LOCS` and `OUTLETS` stay in `packages/contract` (the server reads them). `packages/contract/src/fixtures/master.ts`'s re-export lines for all three are deleted; `UI/src/data/master.ts` re-exports only `ALL_LOCS`, `OUTLETS`, `PO_APPROVAL_LIMIT` now — `PAR_FACTOR` comes from `@rch/domain` directly (`UI/src/lib/selectors.ts`), and `STAFF_CREDIT_LIMIT` was dropped from this file entirely (the till reads the ceiling off `GET /reports/credit/:kind/:id`'s own `limit`, not a bundled constant). | `PAR_FACTOR` has no server-side rule reading it — it is UI tuning, the same class of constant every other file in `packages/domain` already holds, not a wire shape. `STAFF_CREDIT_LIMIT` moving to a live read rather than a constant closes the same class of drift the credit report itself exists to fix — a number the till trusts should be the number the server will actually refuse on. |
| §10 UI | `auth` (`store/index.ts`) gains a fifth state, `"failed"`. A sign-in or a reload whose `GET /snapshot` fails (not a 401 — `onSessionLost` already covers that) renders a full-page retry instead of falling back to `"ready"` with "Could not load the latest data — showing what is in memory." | There is no memory to fall back to any more — `data/seed.ts` is gone — so the old sentence described a state that stopped being true the moment the browser's own copy of the hospital was deleted. |
| §9.2/§10 production | The kitchen's outbound handover gains an OTP field and a labelled-override choice, the same as the store's issue desk — a new `roles/prod/TicketDrawer.tsx` (registered as drawer `"ptkt"`), opened from the kitchen's board (`Dashboard.tsx`), `MakeDistribute.tsx` and `Tickets.tsx` alike, rather than each screen calling `handover(t.id)` with no OTP unconditionally. | Every kitchen handover ran silently as the supervisor override before this landed — not a deliberate choice, an oversight the screen never surfaced a control for. |
| §10 UI | The manager's transfers table (`roles/manager/ItemsStock.tsx`) gains `Cancelled` as a selectable stage filter (`TSTATES`), alongside Reserved / In transit / Received, and both the outbound and inbound cells for a cancelled ticket answer its real status rather than showing a stale "on the shelf"/"holding a code" line. | The label already rendered correctly for a cancelled shop-to-shop ticket in the general case; only the filter list and the two specific cells had not caught up to the status the table could already show. |
| §11.4 | `.github/workflows/deploy.yml`'s `deploy` job gains a job-level `timeout-minutes: 45`, and its "Prove it came up" step uses `kubectl -n "$NS" wait --for=condition=Available` on both Deployments in place of a throwaway pod that pulled `curlimages/curl` from Docker Hub into the namespace to ask the same question. | Found in the whole-branch review: the step as first written had no bound on how long a hung pull, a wedged pod, or an unreachable Docker Hub could hold the runner or the concurrency group open. |
| §13 e2e | `E2E_OTP_REDACTED` and `E2E_SUPPORT_SERVER` — the two switches that gated `e2e/tests/{request-chain,support}.spec.ts`'s assertions behind server-side support for the OTP redaction and the support desk — are gone from `ci.yml`'s kind-install step and from the specs themselves; the assertions they gated are now unconditional. | Both features they were waiting for are on `develop`; a switch nothing turns off any more is dead weight in the CI config and an invitation to wonder what it still does. |
| §12 `loadcheck.mjs` | The password comes from `LOADCHECK_PASSWORD` (`--password` still works, with a warning that it is left in the shell history and `ps`); a numeric flag with no value after it is a usage error, not a `NaN` that runs for zero milliseconds and reports PASS off zero samples. | A load-test script's own invocation should not leak a password to every process listing on the box it runs from, and a silently-wrong flag should not report a false pass. |
| §9.1 `reports/repo.ts` | `stockLedger`'s repo query orders its pick deterministically, so the report's own literal test assertions rest on a named row rather than on whichever one Postgres happened to return first. | Found while hardening the report's own test suite; an unordered read that happens to pass today is exactly the kind of test that goes red on an unrelated Postgres version bump. |
| §9.1 `support` | `GET /support/tickets`'s scoping (the list and its owners) now comes off one read of `support_tickets`, not two. | The list and the owner lookup were two separate queries; a ticket raised between them arrived in the list with no owner recorded against it — invisible to the very person who had just raised it. |
