...
- **A test that races two transactions warms the pool first.** `pg` opens connections lazily, so the second of two "concurrent" `withTransaction` calls waits ~5 ms for a socket and begins after the first has committed — the race never happens and the test passes with the lock removed. Call `warmPool(t, n)` (`apps/api/src/test/db.ts`, Task 4) before racing `n` transactions, and prove each such test fails with the lock or the sort commented out before keeping it.
