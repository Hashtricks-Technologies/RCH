...
import { CurrentShiftResponseSchema, KitchenReportQuerySchema, KitchenReportSchema, ShiftReportSchema, ShiftReportsResponseSchema, ShiftsQuerySchema } from "./schemas/reports.js";
...
import { CreateWastageBodySchema, OutletPricesResultSchema, SaveOutletPricesBodySchema, WastageResultSchema } from "./schemas/writes.js";
...
export const routes = {
  ...
  // ---- kitchen wastage. The kitchen's raw materials and packaging are used the moment they land,
  // so a loss there is recorded (`WST-`), not written off a shelf that holds nothing. The route is
  // the kitchen's door (Adjustments at edit, held to the kitchen by the handler); the record and
  // what the kitchen was issued are read back together, over a window, by `kitchenReport`.
  createWastage:    defineRoute({ method: "POST", path: "/wastage", access: need("adjustments", "edit"), body: CreateWastageBodySchema, response: writeResponse(WastageResultSchema) }),
  kitchenReport:    defineRoute({ method: "GET",  path: "/reports/kitchen", access: need("kitchen_stock", "view"), query: KitchenReportQuerySchema, response: KitchenReportSchema }),
