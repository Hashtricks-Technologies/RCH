...
describe("POST /contracts", () => {
  it("records a rate against a vendor and an item", async () => {
    const b = (await post("u5", "/contracts", { vendorId: "VN-003", it: "bread", rate: 38, from: "2026-04-01", to: "2027-03-31", moq: 20 })).json();
  ...
  it("refuses a second live contract for the same vendor and item", async () => {
    ...
    const r = await post("u5", "/contracts", { vendorId: "VN-001", it: "milk", rate: 55, from: "2026-04-01", to: "2027-03-31" });
  ...
  it("refuses a window that ends before it starts, and a rate of nothing", async () => {
    expect((await post("u5", "/contracts", { vendorId: "VN-003", it: "oil", rate: 120, from: "2027-03-31", to: "2026-04-01" })).json().error.message)
      .toBe("A contract cannot end before it starts");
    expect((await post("u5", "/contracts", { vendorId: "VN-003", it: "oil", rate: 0, from: "2026-04-01", to: "2027-03-31" })).json().error.message)
      .toBe("A contract rate must be more than zero");
  });

  it("404s an unknown vendor or item, and is absent for every role but the procurement officer", async () => {
    expect((await post("u5", "/contracts", { vendorId: "VN-777", it: "oil", rate: 1, from: "2026-04-01", to: "2027-03-31" })).json().error.message)
      .toBe("There is no vendor VN-777.");
    expect((await post("u5", "/contracts", { vendorId: "VN-003", it: "nope", rate: 1, from: "2026-04-01", to: "2027-03-31" })).json().error.message)
  ...
  it("records one contract, not two, when the same pair is added twice at once", async () => {
    ...
    const both = await Promise.all([post("u5", "/contracts", body), post("u5", "/contracts", body)]);
...
describe("PATCH and DELETE /contracts/:id", () => {
  ...
  it("closes a contract without deleting it, and lets it be reopened", async () => {
    ...
    const b = (await del("u5", `/contracts/${id}`)).json();
    ...
    expect((await patch("u5", `/contracts/${id}`, { active: true })).json().result.active).toBe(true);
  });
  ...
  it("will not reopen one into a pair that already has a live contract", async () => {
    ...
    const r = await patch("u5", `/contracts/${id}`, { active: true });
    expect(r.statusCode).toBe(422);
    expect(r.json().error.message).toBe(`${(await getItems()).milk.n} already has a live contract with Aavin Dairy Depot`);
  });
  ...
  it("gives the live slot to exactly one screen when two closed contracts race to reopen it", async () => {
    ...
    const both = await Promise.all([
      patch("u5", `/contracts/${a}`, { active: true }),
      patch("u5", `/contracts/${b}`, { active: true }),
    ]);
  ...
  it("refuses a window that ends before it starts on a patch too, and 404s an unknown id", async () => {
    ...
    expect((await patch("u5", `/contracts/${id}`, { to: "2026-01-01" })).json().error.message).toBe("A contract cannot end before it starts");
    expect((await patch("u5", "/contracts/RC-777", { rate: 1 })).json().error.message).toBe("There is no rate contract RC-777.");
  });
  ...
  it("refuses an empty patch, like vendors does", async () => {
    ...
    expect((await patch("u5", `/contracts/${id}`, {})).json().error.message).toBe(`Nothing to change on ${id}`);
  });
});
