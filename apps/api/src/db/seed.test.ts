...
describe("seed", () => {
  it("loads every master table", async () => {
    ...
    // The three registers a non-cash bill may be posted to, in one table. A consultant the
    // counter can pick but the server cannot find is a bill it would refuse, so the lists match.
    expect(await count(payers)).toBe(FX.STAFF.length + FX.DEPTS.length + FX.DOCTORS.length);
