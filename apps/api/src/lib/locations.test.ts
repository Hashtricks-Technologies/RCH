...
describe("a closed outlet", () => {
  ...
  it("asks for no new product, has no adjustment request raised, no product listed and nothing switched off", async () => {
    await setOpen("kiosk", false);
    expect(refusal(await send("u2", "POST", "/product-requests", { name: "Mango lassi", forLoc: "kiosk" }))).toBe(CLOSED);
    expect(refusal(await send("u6", "POST", "/adjustment-requests", { reason: "count", lines: [{ it: "chips", qty: 1 }] }))).toBe(CLOSED);
